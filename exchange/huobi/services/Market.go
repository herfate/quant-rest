...
import (
	...
	"github.com/findthefirst/quant-rest/exchange/huobi/config"
	"github.com/findthefirst/quant-rest/exchange/huobi/models"
	"github.com/findthefirst/quant-rest/exchange/huobi/untils"
)
...
// strOrderID: 订单ID
// return: string
func GetOrderDetail(strOrderID string) CommonReturn {
	strRequest := fmt.Sprintf("/v1/order/orders/%s", strOrderID)
	jsonPlaceReturn := untils.ApiKeyGet(make(map[string]string), strRequest)
	commonReturn := CommonReturn{}
	json.Unmarshal([]byte(jsonPlaceReturn), &commonReturn)
	return commonReturn
}
